...
Det är fortfarande \emph{digital signalbehandling}, men den används numera mest
...
\index{kvantisering}
\index{quantize}
\index{pulskodsmodulation}
\index{Pulse Code Modulation (PCM)}
\index{PCM|see {pulskodsmodulation}}
\label{sampling}
...
\index{DFT|see {diskret fouriertransform}}
\index{FFT}
...
spektrum.
Detta används numera allt oftare för att få en spektrumplot och genom att
lägga flera av dessa efter varandra kan man få de numera allt vanligare
...
accumulator, PA}).
En fasackumulator är inget annat än ett adderingssteg följt av ett fördröjningssteg.
Det är ett extremfall av ett IIR filter, med enbart en pol, som integrerar,
det vill säga ackumulerande effekt.
Värdet ut från denna representerar oscillatorns fas, därav phase accumulator.
